...
QUIZE_DURATION = 30 # (!) Try changing this to 10 or 60.
MIN_DICE = 2
MAX_DICE = 6
...
'''.format(QUIZE_DURATION, REWARD, PENALTY))
input('Press enter to begin')

correctAnswers = 0
incorrectAnswers = 0
startTime = time.time()
while time.time() < startTime + QUIZE_DURATION: # Main game loop
    ...
    for i, (dieLeft, dieTop) in enumerate(topLeftDiceCorners):
        ...
        dieFace = diceFaces[i]
        for dx in range(DICE_WIDTH - 1):
            for dy in range(DICE_HEIGHT - 1):
